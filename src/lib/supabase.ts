...
export async function sendFeedback(input: FeedbackInput): Promise<FeedbackResult> {
  ...
  if (!supabase) {
    console.warn('[Feedback Desktop] Supabase não configurado (VITE_SUPABASE_URL / VITE_SUPABASE_ANON_KEY ausentes).');
    return {
      success: false,
  ...
  try {
    ...
    const appVersion = input.appVersion || 'desktop-1.22.0';
    ...
    if (error) {
      console.error('[Feedback Desktop] Erro ao gravar feedback no Supabase:', error.message);
      return {
        success: false,
    ...
    console.error('[Feedback Desktop] Exceção durante envio:', err);
    return {
      success: false,
      error: 'Não foi possível enviar seu feedback agora. Tente novamente.',
    };
